        ...
        <shelfSection>
          <items>
            ...
            <sixteenByNinePoster id="shelf_resume_item_<%- index %>" accessibilityLabel="<%= item.Name %>" showOutline="true" alwaysShowTitles="true"
                onSelect="atvutils.loadURL('<%=: [params.serverId, params.libType, params.libId, item.Type.toLowerCase(), item.Id] | buildUrl:{seriesId: item.SeriesId, seasonId: item.ParentId} %>');" 
                onPlay="atvutils.loadURL('<%=: [params.serverId, params.libType, params.libId, item.Type.toLowerCase(), item.Id, 'play'] | buildUrl:{seriesId: item.SeriesId, seasonId: item.ParentId} %>');">
