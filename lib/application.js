...
var logger = new Logger("Main", false);
var atvLogger = new Logger("ATVLogger", true);
...
Application.prototype.getClient = function (serverId) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (srv.client && srv.client.isLoggedIn()) {
            resolve(srv.client);
        } else {
            srv.client = new EmbyClient(srv.Address, srv.UserName || "", srv.Password || "");
            if (!srv.UserName) {
                reject({
                    status : 401,
...
Application.prototype.initRouting = function () {
    ...
    express.post("/login", function (req, res) {
        ...
        if (options.serverId && options.user) {
            var srv = that.getServerById(options.serverId);
            srv.UserName = options.user;
            srv.Password = options.password;

            that.getClient(options.serverId).then(function (embyClient) {
                res.status(301).send(options.redirectURL || "/" + options.serverId + "/");
            }, function () {
                res.status(301).send("/settings/");
            });
        } else {
            res.status(301).send("/settings/");
        }
    ...
    express.post("/player", function (req, res) {
        var options = req.body;

        that.getClient(options.serverId).then(function (embyClient) {
            if (options.state === "start") {
                embyClient.playbackStart(options.mediaSourceId).then(function (result) {
                    res.status(200).send("OK");
                });
            } else if (options.state === "stopped") {
                embyClient.playbackStopped(options.mediaSourceId, options.positionTicks).then(function (result) {
                    res.status(200).send("OK");
                });
            } else if (options.state === "progress") {
                embyClient.playbackProgress(options.mediaSourceId, options.positionTicks).then(function (result) {
                    res.status(200).send("OK");
                });
            } else if (options.state === "paused") {
                embyClient.playbackProgress(options.mediaSourceId, options.positionTicks, "pause").then(function (result) {
                    res.status(200).send("OK");
                });
            }
        }, function (error) {
    ...
    express.get("/:serverId/:libType/:libId/:itemType/:itemId/:view?", function (req, res) {
        ...
        that.getClient(params.serverId).then(function (embyClient) {
            ...
            if (utils.equalsIgnoreCase(params.view, "markWatched")) {
                embyClient.markPlayed(params.itemId).then(function (result) {
                    logger.debug(result);
                    res.status(200).send(result);
                }, function (error) {
                    logger.error(error.message || error);
            ...
            } else if (utils.equalsIgnoreCase(params.view, "markUnwatched")) {
                embyClient.markUnplayed(params.itemId).then(function (result) {
                    logger.debug(result);
                    res.status(200).send(result);
                }, function (error) {
                    logger.error(error.message || error);
                        ...
                        promises.push({
                            promise : utils.imgBackgroundDominant(embyClient.getImageUrl(params.itemId, {
                                ...
                                return new Promise(function (resolve) {
                                    resolve({
                                        titleColor : palette["Vibrant"].getTitleTextColor(),
                                        metadataColor : palette["Vibrant"].getBodyTextColor()
                                    })
                                });
                            }),
                            optional : true,
                ...
                } else if (utils.equalsIgnoreCase(params.itemType, "genre")) {
                    promises.push({
                        promise : embyClient.getItems(undefined, {
                            genreIds : params.itemId,
                ...
                } else if (utils.equalsIgnoreCase(params.itemType, "folder") || utils.equalsIgnoreCase(params.itemType, "photoalbum")) {
                    promises.push({
                        promise : embyClient.getItems(params.itemId, {
...
Application.prototype.run = function (params) {
    ...
    EmbyClient.findServers(5000, function (servers) {
        if (servers.length !== 0) {
            ...
            if (that._params.emby) {
                that._servers = [ {
                    Address : that._params.emby,
                    client : new EmbyClient(that._params.emby)
                } ];

                that._servers[0].client._useProxy = true;
